...
namespace MarlinTrk{
  ...
  int createFinalisedLCIOTrack( IMarlinTrack* marlinTrk, std::vector<EVENT::TrackerHit*>& hit_list, IMPL::TrackImpl* track, bool fit_backwards, EVENT::TrackState* pre_fit, float bfield_z, double maxChi2Increment=DBL_MAX);
  ...
  int finaliseLCIOTrack( IMarlinTrack* marlinTrk, IMPL::TrackImpl* track, std::vector<EVENT::TrackerHit*>& hit_list, IMPL::TrackStateImpl* atLastHit=0, IMPL::TrackStateImpl* atCaloFace=0);
